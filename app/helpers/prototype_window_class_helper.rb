...
module PrototypeWindowClassHelper
  ...
  def params_for_javascript(params) #options_for_javascript doesn't works fine
    ...
    '{' + params.map {|k, v| "#{k}: #{
            case v
              when Hash then
                params_for_javascript( v )
              when String then
                "'#{v}'"
              else
                v   #Isn't neither Hash or String
            end }"}.sort.join(', ') + '}'
  end
  ...
  def link_to_prototype_window( name, window_id, options = { :windowParameters => {} }, html_options = {} )
    ...
    js_code ="WindowCloseKey.init(); var win = new Window( '#{window_id}', #{params_for_javascript(options) } );  win.showCenter(true);  win.setDestroyOnClose();"
    content_tag(
            "a", name,
            html_options.merge({
                    :href => html_options[:href] || "#",
